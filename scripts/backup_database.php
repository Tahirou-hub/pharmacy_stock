...
if ($returnVar === 0 && file_exists($backupFile) && filesize($backupFile) > 0) {
    ...
    // Logger le succès
    $logFile = __DIR__ . '/../logs/backup.log';
    $logMessage = date('Y-m-d H:i:s') . " - Sauvegarde réussie : " . basename($compressedFile) . " (" . number_format(filesize($compressedFile) / 1024, 2) . " KB)\n";
    file_put_contents($logFile, $logMessage, FILE_APPEND);
    
    echo "Sauvegarde réussie : " . basename($compressedFile) . "\n";
    exit(0);
} else {
    // Logger l'erreur
    $logFile = __DIR__ . '/../logs/backup.log';
    $errorMsg = !empty($output) ? implode("\n", $output) : "Erreur inconnue";
    $logMessage = date('Y-m-d H:i:s') . " - ERREUR de sauvegarde : " . $errorMsg . "\n";
    file_put_contents($logFile, $logMessage, FILE_APPEND);
